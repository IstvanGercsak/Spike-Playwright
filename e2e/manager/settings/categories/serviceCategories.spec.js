...
test('Service Categories screen is available', async ({ page }) => {
    await page.locator(navigation.manager()).click()
    await page.locator(navigation.categories()).click()
    await page.getByText(navigation.serviceCategories()).click()
    await expect(page).toHaveTitle('My Phorest');
    await expect(page).toHaveURL(/service-categories/);
});